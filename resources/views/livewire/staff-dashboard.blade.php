<div wire:poll.10s="checkNewTickets">
    ...
    <!-- Real-time Notification Toast & Audio (White Glassmorphism) -->
    <audio id="notification-sound" src="https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3" preload="auto"></audio>
    
    <div 
        x-data="{ show: false, message: '', progress: 100, interval: null }"
        x-on:new-ticket-alert.window="
            message = $event.detail.title;
            show = true;
            progress = 100;
            clearInterval(interval);
            document.getElementById('notification-sound').play().catch(e => console.log('Audio error:', e));
            
            // Progress bar animation (5000ms total)
            interval = setInterval(() => {
                progress -= (100 / 500); // decrement every 10ms
                if(progress <= 0) {
                    clearInterval(interval);
                    show = false;
                }
        ...
        class="fixed bottom-4 right-4 sm:top-6 sm:bottom-auto sm:right-6 z-[100] max-w-sm w-full bg-white/80 backdrop-blur-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl pointer-events-auto border border-white/60 overflow-hidden"
        style="display: none;"
    >
        <div class="p-5 relative">
            <div class="flex items-start gap-4">
                <div class="flex-shrink-0 pt-0.5">
                    <div class="h-10 w-10 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 border border-blue-200/50 flex items-center justify-center shadow-inner">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="mt-1 text-sm font-medium text-slate-500 line-clamp-2" x-text="message"></p>
                </div>
                <div class="flex-shrink-0 flex ml-2">
                    <button @click="show = false; clearInterval(interval)" class="bg-transparent rounded-full p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100/50 focus:outline-none transition-colors">
        ...
        <!-- Progress Bar -->
        <div class="h-1 w-full bg-slate-100/50">
            <div class="h-full bg-gradient-to-r from-blue-400 to-indigo-500" :style="`width: ${progress}%`"></div>
        </div>
    </div>
</div>
